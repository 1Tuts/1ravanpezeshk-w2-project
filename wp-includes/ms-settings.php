...
if ( !isset( $current_site ) || !isset( $current_blog ) ) {
	...
	if ( false !== strpos( $domain, ':' ) ) {
		if ( substr( $domain, -3 ) == ':80' ) {
			...
			wp_die( /*WP_I18N_NO_PORT_NUMBER*/'قابلیت چندسایته فقط بدون شماره‌ی درگاه در نشانی کار می‌کند.'/*/WP_I18N_NO_PORT_NUMBER*/ );
		}
	}
	...
	if ( ! $blog_id ) {
		if ( defined( 'WP_INSTALLING' ) ) {
			...
			$msg = ! $wpdb->get_var( "SHOW TABLES LIKE '$wpdb->site'" ) ? ' ' . /*WP_I18N_TABLES_MISSING*/'جدول‌های پایگاه داده پیدا نمی‌شوند.'/*/WP_I18N_TABLES_MISSING*/ : '';
			wp_die( /*WP_I18N_NO_BLOG*/'سایتی با این نام در این سیستم موجود نیست.'/*/WP_I18N_NO_BLOG*/ . $msg );
		}
	}
}
