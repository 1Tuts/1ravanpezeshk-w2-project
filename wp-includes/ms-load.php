...
function wpmu_current_site() {
	...
	// Still no dice.
	if ( 1 == count( $sites ) )
		wp_die( sprintf( /*WP_I18N_BLOG_DOESNT_EXIST*/'این سایت وجود ندارد. لطفاً <a href="%s">%s</a> را امتحان کنید.'/*/WP_I18N_BLOG_DOESNT_EXIST*/, $sites[0]->domain . $sites[0]->path ) );
	else
		wp_die( /*WP_I18N_NO_SITE_DEFINED*/'سایتی روی این میزبان تعریف نشده است. اگر شما مدیر این سایت هستید از <a href="http://codex.wordpress.org/Debugging_WPMU">رفع‌اشکال از WPMU</a>  کمک بگیرید.'/*/WP_I18N_NO_SITE_DEFINED*/ );
}
...
function ms_not_installed() {
	...
	$title = /*WP_I18N_FATAL_ERROR*/'خطای برقراری ارتباط با پایگاه‌داده'/*/WP_I18N_FATAL_ERROR*/;
	$msg  = '<h1>' . $title . '</h1>';
	if ( ! is_admin() )
		die( $msg );
	$msg .= '<p>' . /*WP_I18N_CONTACT_OWNER*/'اگر سایت شما نمایش داده نمی‌شود، لطفا با مدیر شبکه تماس بگیرید.'/*/WP_I18N_CONTACT_OWNER*/ . '';
	$msg .= ' ' . /*WP_I18N_CHECK_MYSQL*/'اگر این شبکه متعلق به شماست لطفا بررسی کنید که MySQL به درستی کار می‌کند و جدول‌ها خطایی ندارند.'/*/WP_I18N_CHECK_MYSQL*/ . '</p>';
	if ( false && !$wpdb->get_var( "SHOW TABLES LIKE '$wpdb->site'" ) )
		$msg .= '<p>' . sprintf( /*WP_I18N_TABLES_MISSING_LONG*/'<strong>جدول‌های پایگاه داده‌ی شما از دست رفته است.</strong> ممکن است MySQL در حال اجرا نباشد، وردپرس به درستی نصب نشده باشد و یا شخصی آنها را حذف کرده باشد. شما باید همین حالا پایگاه داده‌ی خود را بررسی کنید.'/*/WP_I18N_TABLES_MISSING_LONG*/, $wpdb->site ) . '</p>';
	else
		$msg .= '<p>' . sprintf( /*WP_I18N_NO_SITE_FOUND*/'<strong>سایتی پیدا نشد <code>%1$s</code>.</strong> برای جدول <code>%2$s</code> پایگاه داده را جستجو کنید <code>%3$s</code>. درسته؟'/*/WP_I18N_NO_SITE_FOUND*/, rtrim( $domain . $path, '/' ), $wpdb->blogs, DB_NAME ) . '</p>';
	$msg .= '<p><strong>' . /*WP_I18N_WHAT_DO_I_DO*/'الان چیکار کنم؟'/*/WP_I18N_WHAT_DO_I_DO*/ . '</strong> ';
	$msg .= /*WP_I18N_RTFM*/'صفحه <a target="_blank" href="http://codex.wordpress.org/Debugging_WPMU">رفع‌اشکال از WPMU</a> را بخوانید. بعضی از راهنمایی‌ها ممکن است در پیدا کردن مشکل به شما کمک کنند.'/*/WP_I18N_RTFM*/;
	$msg .= ' ' . /*WP_I18N_STUCK*/'اگر هنوز با این پیام مشکل دارید پایگاه داده‌ی خود را برای شامل بودن جدول‌های زیر بررسی کنید:'/*/WP_I18N_STUCK*/ . '</p><ul>';
